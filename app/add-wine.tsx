...
export default function AddWineScreen({ isInline, draftIdProp, barIdProp, onClose, onSave }: AddWineProps = {}) {
    ...
    const currentStateStr = JSON.stringify({ name, description, vintner, abv, price, selectedCategories, localImages, barId, overrideVisibility, overrideGeneric, overrideSpecific, overrideMeasurement, overridePrep });
    ...
    useTrackRecent(
        ...
        trackedDraft
            ? recentEntry(
                  'wine',
                  trackedDraft.id,
                  trackedDraft.draft_data?.name || name || 'Untitled Wine',
                  {
                      isDraft: true,
    ...
    const handleSaveDraft = async () => {
        try {
            setSaving(true);
            const draftData = { name, description, vintner, abv, price, selectedCategories, localImages, barId, overrideVisibility, overrideGeneric, overrideSpecific, overrideMeasurement, overridePrep };
            const result = await saveDraft({ id: currentDraftId || undefined, entityType: 'wine', draftData });
    ...
    const uploadAndLinkImage = async (uri: string, itemId: string): Promise<string | null> => {
        try {
            ...
            const fileName = `wines/${itemId}/${Date.now()}_${Math.random().toString(36).substring(7)}.${ext}`;
    ...
    const handleSave = () => {
        ...
        if (Platform.OS === 'web') {
            if (window.confirm("Are you sure you want to publish this wine?")) {
                proceed();
            }
        } else {
            Alert.alert(
                "Publish Wine",
                "Are you sure you want to publish this wine?",
                [
    ...
    const performSave = async () => {
        ...
        try {
            ...
            if (insertError || !newWine) throw insertError;
            ...
            for (const img of localImages) {
                if (img.isNew) {
                    const newId = await uploadAndLinkImage(img.url, newWineId);
            ...
            Alert.alert("Error", error.message || "Failed to create wine.");
        } finally {
            setSaving(false);
        }
    };
            ...
            >
                <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 40 }]}>
                    ...
                    <YStack gap="$2" marginBottom="$4">
                        ...
                        <Input
                            value={vintner}
                            onChangeText={(val) => handleCapitalizedChange(val, vintner, setVintner)}
                            onBlur={() => setVintner(capitalize(vintner))}
                            placeholderTextColor="$color11"
