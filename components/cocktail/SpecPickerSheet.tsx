...
export function SpecPickerSheet({
    ...
    const handleScanPhoto = async () => {
        ...
        try {
            ...
            Alert.alert("Could not identify glass", err.message || "Try again with a clearer photo.");
        } finally {
            setScanning(false);
        }
    };
    ...
    const handleConfirmScan = async () => {
        ...
        try {
            ...
            Alert.alert("Error", err.message || "Could not save glassware.");
        } finally {
            setSaving(false);
        }
    };
                ...
                {isGlassware && onIdentifyGlassware && !scanResult && (
                    ...
                    >
                        <XStack gap="$2" alignItems="center" justifyContent="center">
                            {scanning ? (
                                <ActivityIndicator size="small" color={theme.color?.get() as string} />
                            ) : (
                                <IconSymbol name="camera.fill" size={18} color={theme.color?.get() as string} />
                            )}
                            <Text color="$color" fontWeight="600">
                                {scanning ? "Identifying glass…" : "Identify from photo"}
                            </Text>
                        </XStack>
                    </Button>
                )}
                ...
                {scanResult && (
                    ...
                    >
                        ...
                        <XStack gap="$3" justifyContent="flex-end">
                            ...
                            <Button backgroundColor="$color8" onPress={handleConfirmScan} disabled={saving || !scanName.trim()}>
