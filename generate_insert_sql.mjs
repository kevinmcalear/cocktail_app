// This script transforms the ingredient list into a SQL INSERT statement that can be manually run
import fs from 'fs';
...
const values = rawData.map(([name, type, descText]) => {
  ...
  let description = `${descText}\n\n***Caretakers Cottage back bar***`;
  return `('${escapeSqlString(combinedName)}', '${escapeSqlString(description)}')`;
});

sql += values.join(',\n') + `\nON CONFLICT (name) DO UPDATE SET description = EXCLUDED.description;\n`;

fs.writeFileSync('insert_ingredients.sql', sql);
console.log('SQL generated to insert_ingredients.sql. You can copy-paste it to Supabase SQL editor.');
