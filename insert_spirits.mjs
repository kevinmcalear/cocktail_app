...
async function insertIngredients() {
  ...
  for (const [name, type, descText] of rawData) {
    ...
    toInsert.push({ name: combinedName, description: description });
  }

  console.log(`Prepared ${toInsert.length} ingredients to insert.`);

  // Insert in chunks or all at once since it's < 100 rows
  const { data, error } = await supabase
    .from('ingredients')
    .upsert(toInsert, { onConflict: 'name' }) // Upserting by name prevents duplicates if script is run multiple times
    .select('name');

  if (error) {
    console.error("Failed to insert ingredients:", error);
  } else {
    console.log(`Successfully inserted/updated ${data.length} ingredients.`);
  }
}
